...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _login() async {
    if (identity.text.trim().isEmpty || password.text.isEmpty)
      return showMessage(context, 'Lengkapi email/username dan kata sandi.');
    setState(() => waiting = true);
    try {
      final user = await MongoService.instance.login(
        identity.text,
        password.text,
      );
      if (!mounted) return;
    ...
    child: Column(
      ...
      children: [
        ...
        Align(
          ...
          child: TextButton(
            onPressed: () => pushPage(context, const OtpPage()),
            child: const Text(
              'Lupa kata sandi?',
              style: TextStyle(color: alpenGreen, fontWeight: FontWeight.w700),
            ),
          ),
        ),
        ...
        Center(
          child: Wrap(
            children: [
              ...
              GestureDetector(
                onTap: () => pushPage(context, const RegisterPage()),
                child: const Text(
                  'Daftar',
                  style: TextStyle(
                    color: alpenBlue,
