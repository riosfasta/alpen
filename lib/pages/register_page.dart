...
class _RegisterPageState extends State<RegisterPage> {
  ...
  @override Widget build(BuildContext context) => AuthScaffold(showBack: true, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    AppField(label: 'Nama Lengkap', controller: name, hint: 'Masukkan nama lengkap'), AppField(label: 'Email', controller: email, hint: 'Masukkan email', keyboard: TextInputType.emailAddress), AppField(label: 'Nomor Pegawai', controller: username, hint: 'ex: 11921244', keyboard: TextInputType.number), AppField(label: 'Kata Sandi', controller: password, hint: 'Masukkan kata sandi', obscure: true), AppField(label: 'Konfirmasi Kata Sandi', controller: confirm, hint: 'Masukkan ulang kata sandi', obscure: true), const SizedBox(height: 8), PrimaryButton(label: 'Daftar', waiting: waiting, onTap: _submit), const SizedBox(height: 14), const Center(child: Text('Sudah punya akun? Masuk', style: TextStyle(color: Color(0xFF416F54)))),
  ]));
}
